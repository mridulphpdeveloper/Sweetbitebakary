...
mysqli_query($con,"insert into orders(userId,productId,quantity,ordernote) values('".$_SESSION['id']."','$qty','$val34','$ordernote')");
...
<form name="cart" id="cart-form" method="post">	
<?php
if(!empty($_SESSION['cart'])){
	?>
...
$_SESSION['pid']=$pdtid;
	...
	<table class="table table-bordered">
		<thead>
			<tr>
				<th>
					<span class="estimate-title">Order Note</span>
				</th>
			</tr>
		</thead>
		<tbody>
				<tr>
					...
					<input class="form-control" type="text" name="order_notes" placeholder="Order Note.....">
...
<?php include('includes/footer.php');?>
    ...
    function validateShippingAddress() {
        var shippingAddress = "<?php $qry=mysqli_query($con,'select shippingAddress from users where id='.$_SESSION['id'].''); 
            if ($rt=mysqli_fetch_array($qry)) { echo htmlentities($rt['shippingAddress']); } ?>";
    ...
    <script>
        $(document).ready(function(){ 
            ...
            if ($.fn.switchstylesheet) {
                ...
                console.error("switchstylesheet function is not defined.");
            }
